...
public class ScoreboardManager {
    ...
    public ScoreboardManager() {
        ...
        glowingTask = ReliHub.getInstance().getScheduledExecutorService().scheduleAtFixedRate(() ->
        {
            String ip = colorIpAt();
            for (PersonalScoreboard scoreboard : scoreboards.values())
                ReliHub.getInstance().getExecutorMonoThread().execute(() -> scoreboard.setLines(ip));
        }, 80, 80, TimeUnit.MILLISECONDS);

        reloadingTask = ReliHub.getInstance().getScheduledExecutorService().scheduleAtFixedRate(() ->
        {
            for (PersonalScoreboard scoreboard : scoreboards.values())
                ReliHub.getInstance().getExecutorMonoThread().execute(scoreboard::reloadData);
